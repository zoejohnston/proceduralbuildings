...
{   
    List<Brick> brickPool = new List<Brick>();
    List<Quoin> quoinPool = new List<Quoin>();

    private Brick NewBrick(BuildingPart buildingPart) {
        int numPooledBricks = brickPool.Count;
        if (numPooledBricks > 0) {
            Brick newBrick = brickPool[numPooledBricks - 1];
            newBrick.gameObject.SetActive(true);
            brickPool.RemoveAt(numPooledBricks - 1);
            return newBrick;
        } else {
            return Instantiate(buildingPart.brick);
        }
    }

    private Quoin NewQuoin(BuildingPart buildingPart) {
        int numPooledQuoins = brickPool.Count;
        if (numPooledQuoins > 0) {
            Quoin newQuoin = quoinPool[numPooledQuoins - 1];
            brickPool.RemoveAt(numPooledQuoins - 1);
            return newQuoin;
        } else {
            return Instantiate(buildingPart.quoin);
        }
    }

    public void AddToBrickPool(Brick brick) {
        brick.transform.rotation = Quaternion.identity;
        brick.gameObject.SetActive(false);
        brickPool.Add(brick);
    }

    public void AddToQuoinPool(Quoin quoin) {
        quoin.transform.rotation = Quaternion.identity;
        quoin.gameObject.SetActive(false);
        quoinPool.Add(quoin);
    }
    ...
    public void SetNoise(float[,] noise, int numBricksTall, int numBricksWide, int otherInput)
    {
    ...
    public void BuildBrickWall(
        ...
        for (int i = 0; i < numBricksTall; i++)
        {
            ...
            {
                Brick newBrick = NewBrick(buildingPart);
    ...
    public void InitQuoins(float xFlip, float height, float zFlip, BuildingPart buildingPart) {
        Vector3 innerScale = buildingPart.GetScale();
        ...
        for (int i = 0; i < numBricksTall; i++)
        {
            ...
            Quoin newQuoin = NewQuoin(buildingPart);
    ...
    public void MoreQuoins(float x, float z, BuildingPart buildingPart)
    {
        Vector3 innerScale = buildingPart.GetScale();
        Quoin newQuoin = NewQuoin(buildingPart);
        float positionOnRoof = 1.0f - (((innerScale.x / 2.0f) - 0.05f) / (innerScale.x / 2.0f));
    ...
    /// <param name="buildingPart">.</param>
    public void MoreBricks(float width, float side, BuildingPart buildingPart)
    {
        Vector3 innerScale = buildingPart.GetScale();
